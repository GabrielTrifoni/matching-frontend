...
export default function Interest({ projectId }) {
    ...
    async function fetchInterests() {
      try {
        const response = await axios.get(
          `http://localhost:3000/interests/project/${projectId}`,
          {
            headers: {
  ...
  const handleInterestApproval = async (interestId, isApproved) => {
    ...
    try {
      await axios.patch(
        `http://localhost:3000/interests/${interestId}/updateStatus`,
        { status: newStatus },
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  return (
    <div>
      {interests.length > 0 ? (
        interests.map((item) => (
          <div key={item.id}>
            <span>Aluno: {item.user.fullname}</span>
            <br />
            <span>Motivo de interesse: {item.reason}</span>
            <br />
            {item.status === "EM_ANALISE" ? (
              ...
              <p>
                Status: {item.status === "APROVADO" ? "Aprovado" : "Reprovado"}
              </p>
            )}
          </div>
        ))
